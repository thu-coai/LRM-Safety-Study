...
def load_data(data_name, split, data_dir='./data'):
    ...
    if os.path.exists(data_file):
        ...
        if data_name == "math":
            with open(f"{data_dir}/math/math.jsonl", "r") as f:
                dataset = [json.loads(line) for line in f]
        elif data_name == "math500":
            with open(f"{data_dir}/math500/math500.jsonl", "r") as f:
                dataset = [json.loads(line) for line in f]
        elif data_name == "aime_24":
            with open(f"{data_dir}/aime_24/test2024.jsonl", "r") as f:
                dataset = [json.loads(line) for line in f]
